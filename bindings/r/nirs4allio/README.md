...
R binding for [`nirs4all-io`](../../..), the Rust-first low-level reader library
...
`nirs4all-io` does no modelling itself; it produces the records that the
...
package still installs and falls back to invoking the `nirs4all-io` command-line
tool (see *Transport* below).

The optional `tibble` package enables `nirs4allio_as_tibble()`.

## Transport: native vs CLI

| Path | When it is used | Capabilities |
|------|-----------------|--------------|
| **Native (extendr)** | Built at install time with Cargo on `PATH` | All functions, including in-memory `nirs4allio_open_bytes()` and `nirs4allio_open_with_sidecars()` |
| **CLI fallback** | Native library absent | File-path reads, `nirs4allio_probe_path()`, `nirs4allio_walk_path()` |

Use `nirs4allio_native_available()` to check which path is active.

CLI resolution order:

1. `NIRS4ALL_IO_CLI` environment variable, if set (whitespace-split into a
   command plus arguments);
2. a `nirs4all-io` binary found on `PATH`;
3. in a source checkout, `cargo run -p nirs4all-io-cli`.
...
if (nirs4allio_native_available()) {
  ...
  recs <- nirs4allio_open_bytes("spectrum.dx", raw_bytes)
}
```

## The `nirs4allio_dataset` object

`nirs4allio_open_dataset()` returns an object of class `nirs4allio_dataset`, a
named list with:
...
`nirs4allio_open_records()` directly.
...
| Function | Purpose |
|----------|---------|
| `nirs4allio_open_records(path)` | Lossless records as nested R lists. |
| `nirs4allio_open_dataset(path, signal = NULL)` | Flat `nirs4allio_dataset`. |
| `as.matrix(dataset)` | Spectral matrix from a dataset. |
| `as.data.frame(dataset)` | Wide data frame from a dataset. |
| `nirs4allio_as_tibble(dataset)` | Dataset as a tibble. |
| `nirs4allio_probe_path(path)` | Ordered candidate readers for a file. |
| `nirs4allio_walk_path(path, ...)` | Recursive per-file scan outcomes. |
| `nirs4allio_open_bytes(name, bytes)` | Decode in-memory bytes (native only). |
| `nirs4allio_open_with_sidecars(name, bytes, sidecars)` | Decode bytes plus companion files (native only). |
| `nirs4allio_native_available()` | Whether the native backend is loaded. |
| `nirs4allio_version()` | Binding version string. |

See `?nirs4allio` and the per-function help pages (e.g. `?nirs4allio_open_dataset`)
for full details.
