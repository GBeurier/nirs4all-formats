...
#' @description
#' Decodes a single file through the Rust `nirs4all-io` registry and returns the
...
#' model. For a flat spectral matrix use [nirs4allio_open_dataset()].
#'
...
#' through the `nirs4all-io` CLI (see *Transport* in [nirs4allio_open_dataset()]).
#'
...
#' }
#'
#' @seealso [nirs4allio_open_dataset()] for a flat matrix view,
#'   [nirs4allio_probe_path()] to inspect candidate readers,
#'   [nirs4allio_walk_path()] to scan a directory.
#' @export
...
#' Read a file into an `nirs4allio_dataset`
#'
#' @title Read a spectroscopy file into a flat spectral dataset
#' @description
#' Loads a file with [nirs4allio_open_records()] and projects one signal per
...
#' from [nirs4allio_open_records()] directly.
#'
...
#' the `nirs4all-io` CLI: the `NIRS4ALL_IO_CLI` environment variable may point
...
#'
#' @return An object of class `nirs4allio_dataset`: a named list with
#'   \describe{
...
#'   }
#'   Use [as.matrix()] / [as.data.frame()] / [nirs4allio_as_tibble()] to project
...
#' ds_abs <- nirs4allio_open_dataset("spectrum.dx", signal = "absorbance")
#' }
#'
#' @seealso [nirs4allio_open_records()] for the lossless record view,
#'   [as.matrix.nirs4allio_dataset()], [as.data.frame.nirs4allio_dataset()],
...
#' Coerce an `nirs4allio_dataset` to a spectral matrix
#'
#' @title Extract the spectral matrix from a dataset
#' @description
#' [as.matrix()] method for [nirs4allio_dataset][nirs4allio_open_dataset]
...
#'
#' @param x An `nirs4allio_dataset` from [nirs4allio_open_dataset()].
...
#' }
#'
#' @seealso [nirs4allio_open_dataset()], [as.data.frame.nirs4allio_dataset()].
#' @exportS3Method base::as.matrix
as.matrix.nirs4allio_dataset <- function(x, ...) {
  x$x
}

#' Coerce an `nirs4allio_dataset` to a data frame
#'
#' @title Build a wide data frame from a dataset
#' @description
#' [as.data.frame()] method for [nirs4allio_dataset][nirs4allio_open_dataset]
...
#' }
#'
#' @seealso [nirs4allio_open_dataset()], [as.matrix.nirs4allio_dataset()],
#'   [nirs4allio_as_tibble()].
...
#' Coerce an `nirs4allio_dataset` to a tibble
#'
#' @title Convert a dataset to a tibble
#' @description
#' Convenience wrapper that converts an
#' [nirs4allio_dataset][nirs4allio_open_dataset] to a
#' [tibble][tibble::tibble] via [as.data.frame.nirs4allio_dataset()]. The
#' optional `tibble` package must be installed.
#'
#' @param dataset An `nirs4allio_dataset` from [nirs4allio_open_dataset()].
#'
#' @return A [tibble::tibble] with the same columns as
#'   [as.data.frame.nirs4allio_dataset()] (`sample_id`, target columns,
#'   `x_<wavelength>` spectral columns).
#'
#' @examples
#' \dontrun{
#' ds <- nirs4allio_open_dataset("samples/csv_tsv/synthetic_nirs.csv")
#' nirs4allio_as_tibble(ds)
#' }
#'
#' @seealso [nirs4allio_open_dataset()], [as.data.frame.nirs4allio_dataset()].
#' @export
...
#' when present; otherwise the `nirs4all-io probe` CLI command is invoked (see
#' *Transport* in [nirs4allio_open_dataset()]).
#'
...
#' @examples
#' \dontrun{
#' probes <- nirs4allio_probe_path("samples/csv_tsv/synthetic_nirs.csv")
#' probes[[1]]$format
#' }
#'
#' @seealso [nirs4allio_open_records()], [nirs4allio_walk_path()].
#' @export
...
#' [nirs4allio_open_with_sidecars()] for those.
#'
...
#'
#' @return A list of records, identical in shape to [nirs4allio_open_records()].
#'
#' @examples
#' \dontrun{
#' bytes <- readBin("spectrum.dx", what = "raw",
#'                  n = file.info("spectrum.dx")$size)
#' records <- nirs4allio_open_bytes("spectrum.dx", bytes)
#' }
#'
#' @seealso [nirs4allio_open_with_sidecars()], [nirs4allio_open_records()],
#'   [nirs4allio_native_available()].
#' @export
...
#' }
#'
#' @seealso [nirs4allio_open_bytes()], [nirs4allio_open_records()],
#'   [nirs4allio_native_available()].
#' @export
...
#' otherwise the `nirs4all-io scan --json` CLI command is invoked and its
#' `entries` are returned (see *Transport* in [nirs4allio_open_dataset()]).
#'
...
#' @examples
#' \dontrun{
#' entries <- nirs4allio_walk_path("samples/asd")
#' length(entries)
#' entries[[1]]$status
#' entries[[1]]$format
#'
#' # Limit recursion depth and include unsupported files
#' nirs4allio_walk_path("samples", max_depth = 1, include_unsupported = TRUE)
#' }
#'
#' @seealso [nirs4allio_probe_path()], [nirs4allio_open_records()].
#' @export
