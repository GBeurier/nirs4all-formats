...
test_that("ENVI Standard cube decodes from in-memory sidecars", {
  skip_if_not(
    ...
    "nirs4allio_open_with_sidecars requires the extendr static library; install via `R CMD INSTALL bindings/r/nirs4allio`."
  )
...
test_that("ERDAS LAN decodes from in-memory sidecars", {
  ...
  records <- nirs4allio_open_with_sidecars("92AV3C.lan", bytes, sidecars)
