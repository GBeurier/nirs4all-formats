#' nirs4allformats: R Binding for nirs4all-formats
#'
#' @description
...
#' invoking the `nirs4all-formats` CLI binary. The `NIRS4ALL_FORMATS_CLI` environment
#' variable may point to a prebuilt CLI binary; in a source checkout the binding
#' can also fall back to `cargo run -p nirs4all-formats-cli`. In-memory decoding
#' ([nirs4allformats_open_bytes()], [nirs4allformats_open_with_sidecars()]) is available
#' only on the native path.
#'
#' @section Main functions:
#' \describe{
#'   \item{[nirs4allformats_open_records()]}{Lossless records as nested R lists.}
#'   \item{[nirs4allformats_open_dataset()]}{Flat `nirs4allformats_dataset` (matrix +
#'     targets + metadata).}
#'   \item{[nirs4allformats_probe_path()]}{Ordered candidate readers for a file.}
#'   \item{[nirs4allformats_walk_path()]}{Recursive per-file scan outcomes.}
#'   \item{[nirs4allformats_open_bytes()] / [nirs4allformats_open_with_sidecars()]}{
#'     In-memory decoding (native backend only).}
#'   \item{[nirs4allformats_native_available()] / [nirs4allformats_version()]}{Backend and
#'     version introspection.}
#' }
#'
#' @seealso [nirs4allformats_open_dataset()], [nirs4allformats_open_records()].
#' @keywords internal
"_PACKAGE"
