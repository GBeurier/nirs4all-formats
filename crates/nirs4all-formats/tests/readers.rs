...
fn assert_sed_signal_units(record: &nirs4all_formats::SpectralRecord, expected: &[&str]) {
...
fn assert_svc_sig_triplet_record(
    record: &nirs4all_formats::SpectralRecord,
    first_axis: f64,
    last_axis: f64,
...
fn assert_svc_signal_units(record: &nirs4all_formats::SpectralRecord) {
    let units = record.metadata["source_signal_units"]
        .as_array()
        .expect("source signal units");
...
fn refuses_witec_wip_binary_projects() {
    ...
    path.push(format!(
        "nirs4all-formats-witec-wip-{}.wip",
        std::process::id()
    ));
