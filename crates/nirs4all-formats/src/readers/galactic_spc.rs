...
impl Reader for GalacticSpcReader {
    fn name(&self) -> &'static str {
        "nirs4all_formats::readers::galactic_spc"
    }
    ...
    fn read_bytes(
        ...
        match bytes.get(1).copied() {
            ...
            Some(0x4c) => Err(nirs4all_formats_core::Error::InvalidRecord(
                "Galactic SPC new big-endian files are recognized but not implemented yet"
                    .to_string(),
            )),
            _ => Err(nirs4all_formats_core::Error::InvalidRecord(
...
fn with_path_context(
    error: nirs4all_formats_core::Error,
    path: &Path,
) -> nirs4all_formats_core::Error {
    match error {
        nirs4all_formats_core::Error::InvalidRecord(message) => {
            nirs4all_formats_core::Error::InvalidRecord(format!("{}: {message}", path.display()))
        }
        other => other,
    }
}
    ...
    if header.flags.txyxys {
        ...
        for _ in 0..header.fnsub {
            ...
            let end = offset.checked_add(size).ok_or_else(|| {
                nirs4all_formats_core::Error::InvalidRecord(
                    "SPC subfile offset overflow".to_string(),
                )
            })?;
    ...
    for _ in 0..header.fnsub {
        let sub_header =
            parse_subheader(bytes.get(offset..offset + SUB_HEADER_LEN).ok_or_else(|| {
                nirs4all_formats_core::Error::InvalidRecord(
                    "SPC subfile header truncated".to_string(),
                )
            })?)?;
        let exp = effective_new_exp(header, &sub_header, warnings);
        ...
        let end = offset.checked_add(size).ok_or_else(|| {
            nirs4all_formats_core::Error::InvalidRecord("SPC subfile offset overflow".to_string())
        })?;
        let data = bytes.get(offset..end).ok_or_else(|| {
            nirs4all_formats_core::Error::InvalidRecord("SPC subfile data truncated".to_string())
        })?;
...
fn y_value_byte_len(count: usize, exp: EffectiveExp, tsprec: bool) -> Result<usize> {
    ...
    count.checked_mul(bytes_per_value).ok_or_else(|| {
        nirs4all_formats_core::Error::InvalidRecord("SPC data byte length overflow".to_string())
    })
}
...
fn le_i16(bytes: &[u8], offset: usize) -> Result<i16> {
    let data = bytes.get(offset..offset + 2).ok_or_else(|| {
        nirs4all_formats_core::Error::InvalidRecord("SPC i16 field truncated".to_string())
    })?;
...
fn le_u32(bytes: &[u8], offset: usize) -> Result<u32> {
    let data = bytes.get(offset..offset + 4).ok_or_else(|| {
        nirs4all_formats_core::Error::InvalidRecord("SPC u32 field truncated".to_string())
    })?;
...
fn le_i32(bytes: &[u8], offset: usize) -> Result<i32> {
    let data = bytes.get(offset..offset + 4).ok_or_else(|| {
        nirs4all_formats_core::Error::InvalidRecord("SPC i32 field truncated".to_string())
    })?;
...
fn le_f32(bytes: &[u8], offset: usize) -> Result<f32> {
    let data = bytes.get(offset..offset + 4).ok_or_else(|| {
        nirs4all_formats_core::Error::InvalidRecord("SPC f32 field truncated".to_string())
    })?;
...
fn le_f64(bytes: &[u8], offset: usize) -> Result<f64> {
    let data = bytes.get(offset..offset + 8).ok_or_else(|| {
        nirs4all_formats_core::Error::InvalidRecord("SPC f64 field truncated".to_string())
    })?;
