...
fn parse_fixed_header(bytes: &[u8]) -> Result<AsdFixedHeader> {
    ...
    let app_data = bytes.get(offset + 203..offset + 331).ok_or_else(|| {
        nirs4all_io_core::Error::InvalidRecord("ASD app data field truncated".to_string())
    })?;
...
fn invalid_record(message: impl Into<String>) -> nirs4all_io_core::Error {
    nirs4all_io_core::Error::InvalidRecord(message.into())
}
...
fn le_i16(bytes: &[u8], offset: usize) -> Result<i16> {
    let data = bytes.get(offset..offset + 2).ok_or_else(|| {
        nirs4all_io_core::Error::InvalidRecord("ASD i16 field truncated".to_string())
    })?;
    Ok(i16::from_le_bytes(
        data.try_into().expect("slice length checked"),
    ))
