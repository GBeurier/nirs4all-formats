...
impl Reader for ParquetReader {
    ...
    fn read_path(&self, path: &Path) -> Result<Vec<nirs4all_io_core::SpectralRecord>> {
        ...
        let layout =
            spectral_layout(schema.fields().iter().enumerate().map(|(index, field)| {
                (index, field.name().to_string(), field.data_type().clone())
            }))?;

        let batch_reader = builder.with_batch_size(1024).build().map_err(|error| {
            Error::InvalidRecord(format!("Parquet batch reader error: {error}"))
        })?;
        let mut records = Vec::new();
        for batch in batch_reader {
            let batch = batch
                .map_err(|error| Error::InvalidRecord(format!("Parquet batch error: {error}")))?;
            for row in 0..batch.num_rows() {
                ...
                if let Some(sample_id_column) = layout.sample_id_column {
                    if let Some(sample_id) =
                        string_value(batch.column(sample_id_column).as_ref(), row)?
                    {
                ...
                for column in &layout.target_columns {
                    let value =
                        target_value(batch.column(column.index).as_ref(), row, &column.name)?;
