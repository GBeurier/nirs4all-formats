...
|---|---|---|---|---|
| `synthetic_nirs.h5` | ~85 KB | Generated locally | CC-0 | A minimally idiomatic multi-signal NIRS HDF5: `/spectra` absorbance and `/reflectance` (both samples × bands, gzip-compressed), shared `/wavelengths`, `/protein`, plus `units` attributes and `instrument` root attribute. |
...
- Schema detection: walk the tree, look for recognized 2-D spectral datasets with one dimension matching a 1-D wavelength or wavenumber axis (`wavelengths`, `wavelength_nm`, `wl`, `lambda`, `wavenumber`, `wn`, `x_axis`, etc.). Compatible datasets in the same group and orientation become multiple signals on each record. If only the first matrix dimension matches the axis, the reader treats the payload as bands × samples.
