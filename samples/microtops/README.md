...
- **NetCDF MAN export** (PANGAEA fixture): `time` is the master dimension; each AOT wavelength is a separate variable (`aot_380`, …, `aot_870`). The loader discovers `aot_<nm>` variables, folds them into `aot` and `aot_std` signals with axis `[380, 440, 500, 675, 870]` nm, and preserves lat/lon/time/air_mass metadata. The MSM114/2 payload is decoded through a generic contiguous-layout fallback (no SHA-256 fixture, no hard-coded byte offsets): the reader scans the file's fractal-heap link records for `<name_len><name><object_header_addr>` triplets, resolves each candidate via `Hdf5File::get_or_parse_header`, and reads `DataLayout::Contiguous` payloads directly. Records carry `microtops_man_netcdf_contiguous_layout_fallback` and `microtops_man_netcdf_global_attributes_byte_scan` whenever those paths fire.
