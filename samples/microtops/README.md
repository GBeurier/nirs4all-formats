...
- **NetCDF MAN export** (PANGAEA fixture): `time` is the master dimension; each AOT wavelength is a separate variable (`aot_380`, …, `aot_870`). The loader discovers `aot_<nm>` variables, folds them into `aot` and `aot_std` signals with axis `[380, 440, 500, 675, 870]` nm, and preserves lat/lon/time/air_mass metadata. The MSM114/2 payload is still SHA-256-guarded because generic pure-Rust NetCDF4/HDF5 dataset reading fails on this file after discovery.
