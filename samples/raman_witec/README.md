...
Native binary `.wip` / `.wid` (WiTec Project file) — **no open reader exists** and no permissively-licensed sample is available on GitHub.
...
- Magic of a real `.wip` (for refusal-path detection): bytes 0-3 are `57 49 54 5e` (`WIT^`) per the few public reverse-engineering notes; **don't depend on this magic without verifying** — no validation fixture available.
- v1 strategy: refuse `.wip` with an explicit error pointing users to WiTec Project FIVE's "Export Spectrum as Text" command, and parse the ASCII export.
