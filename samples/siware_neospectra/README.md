...
| `neospectra_ossl_50samples_slice.csv` | 117 KB | Small slice of `Neospectra_WoodwellKSSL_soil+site_NIR.csv` from the same Zenodo record (full file = 169 MB / 1976 rows) | CC-BY-4.0 | Real 24-record OSSL NeoSpectra slice: mineral-soil samples × 614 columns (KSSL metadata + 9 scanners × replicates + averaged NIR spectrum). Demonstrates the "wide CSV with metadata block + spectra block" layout actually shipped by Woodwell. |
