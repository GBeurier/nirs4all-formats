...
| File | Size | Notes |
|---|---|---|
| `scio_app_scan.csv` | 7 KB | Single-scan export from the SCiO developer app using one wide `band740` ... `band1070` spectral row. Wavelength axis is in the header. |
...
| `scio_scans_from_tech_support.csv` | 1.7 MB | Multi-scan dump shipped by Consumer Physics support, kept as the bulk fixture: 145 measurements with metadata columns followed by grouped `spectrum_*`, `wr_raw_*` and `sample_raw_*` columns. |
...
- The native Rust reader maps `scio_app_scan.csv` and
  `scio_scans_from_tech_support.csv` to `format = scio-csv`; the calibration
  plate is an axis-first table handled by `spectral_table`.
