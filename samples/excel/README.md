...
- Many lab transfers use multi-sheet workbooks with: `metadata` (one row per sample), `spectra` (matrix), `references` (target properties). The loader should let the caller pass sheet names.
