...
CSV / XLSX export from MicroNIR Pro / MicroNIR 1700 / MicroNIR OnSite handheld MEMS-based spectrometer, plus the native `.sam` sample file. Native `.pri` project files are out of scope for v1 (no public spec).

## Samples

| File | Size | Source | License | Notes |
|---|---|---|---|---|
| `synthetic_micronir.sam` | ~1 KB | Generated locally | CC-0 | Synthetic MicroNIR native `.sam`: 1 record, 5 absorbance channels + 6 raw pixels; byte-accurate fixture for the native `viavi_micronir` reader golden. |
...
- Native `.sam` sample file: a .NET-serialised container sniffed by its `\x04MNIR` magic. The synthetic fixture is byte-accurate for the native reader; real native `.sam` vendor files live in `samples_local/viavi_micronir/` (local-only, licence TBD).
