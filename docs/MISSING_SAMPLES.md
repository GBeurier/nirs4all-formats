...
| 3 | `.NIR` / `.cal` native | NIRSystems 5000 / 6500 / XDS / DA / DS | Foss | WinISI II `.cal` (spectra + constituents) and spectra-only `.nir` binary | 🟡 (local) | **Resolved 2026-06-03, kept local** — the native reader sniffs the WinISI header signature (never the extension, so no collision with BUCHI `.nir`) and decodes absorbance spectra, constituent reference targets and acquisition metadata; validated against ISIscan `.txt` exports to float32 precision. Committed CC0 golden `samples/foss_winisi/synthetic.cal`; real vendor files are local-only under `samples_local/foss_winisi/`. `.DA` database and `.eqa` equation companions remain unsampled. | A Foss WinISI II owner for a redistributable fixture and `.DA`/`.eqa` companions. |
...
| 7 | `.pri` project | MicroNIR Pro / MicroNIR OnSite | VIAVI / JDSU | binary project container (post-2018 firmware) | ⚪ | Customer-only; no GitHub mirror. Native single-measurement `.sam` files are now decoded (row 7b); the `.pri` project container is still blocked. | A VIAVI MicroNIR Pro customer. |
| 7b | `.sam` native measurement | MicroNIR 1700 / OnSite | VIAVI / JDSU | .NET-serialised `MNIR` measurement container | 🟡 (local) | **Resolved 2026-06-03, kept local** — the native reader sniffs the length-prefixed `MNIR` magic (resolving the old Galactic-SPC false positive) and decodes calibrated absorbance (125 channels) plus raw single-beam (128 pixels) with acquisition metadata. Committed CC0 golden `samples/viavi_micronir/synthetic_micronir.sam`; real vendor files local-only under `samples_local/viavi_micronir/`. The absorbance wavelength axis is interpolated from the stored endpoints. | A VIAVI MicroNIR customer for a redistributable `.sam`. |
...
| 9 | SiWare API JSON / CSV stream | NeoSpectra Cloud | Spectro Inc. | JSON Web API response payload + companion CSV stream | ⚪ | Synthetic JSON and CSV fixtures are covered; the real API remains credential-gated. | A Spectro Inc. customer who can dump an HTTP response and companion CSV export. |
...
| 10b | Microtops II `.lev10/.lev15/.lev20` ASCII | Microtops II handheld sun-photometer | Solar Light / AERONET MAN | Raw cruise ASCII export with 8-channel AOD + Ångström + WV | 🟡 (local) | **Resolved 2026-05-20, kept local** — AERONET MAN Okeanos Explorer 2019 cruise zip placed in `samples_local/microtops/`; the Rust reader now validates AOD/AOD-STD locally, but AERONET PI-coauthorship policy means we cannot redistribute. | — |
| 11a | MFR-7 / MFRSR real NetCDF | SGP E11 MFRSR 7-channel | YES Inc. / DOE ARM | b1-level NetCDF, 7 narrowband channels | 🟡 (local) | **Resolved locally 2026-05-20, kept local** — `samples_local/mfr/arm_mfrsr_sgp_E11_20210329.nc` is a true ARM MFRSR b1 NetCDF, not a legacy `.OUT`; the Rust reader now loads the 4,320-observation multispectral time series. | Redistributable MFR/MFRSR field export and ACT/xarray conformance. |
| 11b | PP Systems UniSpec-DC real exports | Toolik Lake LTER plots | PP Systems | Multi-year vegetation indices (NDVI/EVI/PRI/etc.) | 🟡 (local, derived) | **Inventoried 2026-05-20, kept local** — Arctic LTER knb-lter-arc.10185.7 derived indices CSV+XLSX in `samples_local/pp_systems/`. These are not raw spectra, so raw `.SPT/.SPU` is still not publicly available. | A PP Systems UniSpec customer for the raw binary. |
| 25a | BUCHI NIRCal real `.nir` with non-null targets | NIRMaster B-30 / N-300 / N-500 / NIRMaster Pro | BUCHI / Buhler | Calibration-transfer file with real cannabinoid targets | 🟡 (local) | **Resolved 2026-05-20, kept local** — orellano-c/transpec_info `DEMO_file_cannabis.nir` (105 spectra x 3 reps x 35 samples) in `samples_local/buchi_nircal/`; the Rust reader now validates non-null `CBDA`/`THCA` targets, replicate metadata and `Spectra Info` metadata locally. | Redistributable non-null target fixture, `.cal`, JCAMP-DX export, NIRMaster/NIRFlex variants. |
| 11c | ARM NetCDF adjacents | AOSMET / SURFSPECALB | DOE ARM | Meteorology and surface-albedo products | 🟡 (local, adjacent) | **Partly resolved locally 2026-05-20, kept local** — `arm_mar_aosmet_20180201.nc` is meteorology and is refused; `arm_nsa_surfspecalb_20160609.nc` is a derived `time x filter` albedo product and now loads as an adjacent spectral product after dropping all-missing rows. | Broader ARM SURFSPECALB validation and a policy for derived spectral products. |
...
| 16 | WiTec `.wip` / `.wid` binary | alpha300 / alpha500 confocal Raman | WiTec | binary project file (any firmware) | ✅ | **Resolved 2026-05-20** — Zenodo 7907659 (ZrO2 Raman imaging, ODbL v1.0) ships `Sa1-Sa5.wip`; `Sa4.wip` (19 MB, smallest) is committed in `raman_witec/`. Detection magic and one `WIT_PR06` TDGraph native layout are validated; broader project-tree decoding remains partial. | — |
...
| 20 | Galactic `.spc` | Old + new LSB headers, -XY/-XYY/-XYXY layouts, NIR/FTIR/Raman/NMR/MS/UV-Vis flavours | **No new big-endian `0x4C` payload**, **no Thermo GRAMS .spc multi-channel (≥3 channels)** — multi-channel is covered analytically by `m_xyxy.spc` but not by a real instrument fixture. | Big-endian GRAMS files are uncommon in the public corpora checked so far; multi-channel instruments are niche. |
...
| 22 | Perkin Elmer | `.sp` (single spectrum), including FT-MIR Spectrum 10 exports with a trailing audit/history block (committed CC0 golden `samples/perkin_elmer/synthetic_trailing.sp`; local-only 31-file MIR corpus with paired CSV exports validated to export precision) | **`.fsm` Spotlight imaging** (50 MB available in `specio` but explicitly skipped). No PE Lambda 1050 NIR-specific blocks. | `.fsm` is 50 MB — too large for fixture; explicitly out of scope for v1 anyway per FORMATS.md §2. |
...
| 25 | BUCHI NIRCal | `.nir` calibration-transfer file (plant tissue) with spectra, cibles nulles, replicats and `Spectra Info` metadata | **No `.cal` calibration-only files**, **no JCAMP-DX export**, **no NIRMaster B-30/N-300/NIRFlex recent vendor variants** | These ship only with the NIRMaster firmware or customer exports. |
| 26 | Avantes AvaSoft 6/7 | `.TRM`, `.ROH`, `.DRK`, `.REF` plus legacy `.IRR` routed as ASCII export | **No binary `.ABS` v6/v7**, **no binary `.IRR` v6/v7 absolute irradiance**, **no `.RMN` Raman v6/v7** | These per-mode binary variants exist in vendor docs but no open fixture. The committed `.IRR` is a text irradiance export, not a binary legacy fixture. |
...
| 30 | SVC / GER `.sig` | Full committed 15-fixture corpus is golden-backed: Acer leaf (SVC HR-1024), Serbin BNL (SVC laptop + SVC moc), BAD fixtures, **GER 3700 PDA raw acquisition** (`serbinsh_gr070214_003.sig`) and **HR-1024i Barrow Environmental Observatory phenology field** (`serbinsh_BEO_CakeEater_Pheno_026_resamp.sig`) | **No SVC HR-1024i firmware ≥3.0 specifically** | GER PDA was retired 2009; the new `serbinsh_gr070214_003.sig` closes the long-standing "no raw GER 3700" gap. |
...
- Remaining synthetic-only gaps are specific export paths: legacy Microtops
  `.TXT`, legacy MFR `.OUT`, PP Systems raw `.SPT/.SPU`, MODTRAN albedo,
  Metrohm Vision Air CSV/native, Shimadzu text/native, Perten DA and
  AnIML/FGI/SiWare API/JASCO Raman variants. Real Microtops MAN NetCDF now
  ships, so the remaining Microtops gap is specifically the legacy `.TXT`
  export.
...
- **Critical missing pieces without a documented text fallback**: ASD `.ILL` /
  `.REF` / `.RAW` calibration triplet, Foss `.DA/.eqa` companions, Perten DA /
  ...
  WiTec `.wip` and Excel handheld exports; the 2026-06-03 round additionally
  decoded the **native** Foss WinISI `.cal/.nir`, VIAVI MicroNIR `.sam` and the
  PerkinElmer FT-MIR Spectrum 10 `.sp` trailing-block variant (each with a
  committed CC0 synthetic golden plus local-only real corpora) — see
  `FORMAT_MATRIX.md` for sources and licences.
