...
The R package (`nirs4allformats`) is a thin wrapper over the Rust core. Parser logic
...
the `nirs4all-formats` CLI (see *Transport*). The optional `tibble` package enables
...
- `nirs4allformats_open_records(path)`: normalized records (one named list per
  record), the lossless mirror of the Rust `SpectralRecord`;
- `nirs4allformats_open_bytes(name, bytes)`: decode an in-memory `raw` buffer
  (native backend only; sidecar formats are rejected);
- `nirs4allformats_open_with_sidecars(name, bytes, sidecars)`: decode bytes plus a
  named list of companion `raw` vectors (native backend only);
- `nirs4allformats_probe_path(path)`: ordered candidate readers without a full parse;
- `nirs4allformats_walk_path(path, ...)`: recursive per-file outcomes.

Each record from `nirs4allformats_open_records()` is a named list with `signals`
...
`nirs4allformats_open_dataset(path, signal = NULL)` projects one signal per record
into a rectangular, R-friendly object of class `nirs4allformats_dataset`. All records
...
S3 methods and a tibble helper turn an `nirs4allformats_dataset` into common R
shapes:
...
- `nirs4allformats_as_tibble(dataset)`: the same wide table as a tibble (requires the
  `tibble` package).

## Walking a directory

`nirs4allformats_walk_path(path, max_depth = NULL, include_hidden = FALSE,
...
`nirs4allformats_open_bytes(name, bytes)` decodes a `raw` buffer through the registry
...
`nirs4allformats_open_with_sidecars(name, bytes, sidecars)`, where `sidecars` is a
...
- otherwise the bridge calls the `nirs4all-formats` CLI: `NIRS4ALL_FORMATS_CLI` can point
...
Use `nirs4allformats_native_available()` to tell which path is active.
`nirs4allformats_open_bytes()` and `nirs4allformats_open_with_sidecars()` are available
