...
| Ocean Optics SpectraSuite / OceanView / Jaz / CRAIC | Ocean Optics / Ocean Insight | `.txt`, `.csv`, `.jaz`, `.JazIrrad`, `.Master.Transmission`, `.ProcSpec`, `.jdx`, `.spc` | textes SpectraSuite/OceanView/Jaz/CRAIC + CSV + ProcSpec; `.jdx` via JCAMP; `.spc` Galactic route | partiel | lightr, pavo |
...
| SVC / GER SIG | Spectra Vista / GER | `.sig` | 15-fixture corpus golden-backed: PDA/laptop/moc + GER 3700 + HR-1024i field; BAD fixtures flagged | partiel | spectrolab, specdal |
...
| Allotrope ADF | Allotrope Foundation | `.adf` | HDF5/RDF; `adfsee` GitLab `example.adf` local data-cube subset + RDF component mapping minimal | partiel | Allotrope SDK, adfsee |
...
| Microtops Sun Photometer | partiel | MAN NetCDF reel committe et teste (PANGAEA MSM114/2, CC-BY-4.0). Le reader tente une decouverte generique `aot_<nm>`, mais le payload MSM114/2 reste lu via fallback SHA-256 car `hdf5-reader` ne resout pas encore ce layout NetCDF4/HDF5. Les 7 exports AERONET MAN ASCII locaux `.lev10/.lev15/.lev20` sont testes avec AOD et AOD-STD. Restent un vrai `.TXT` legacy redistribuable et un lecteur MAN NetCDF generique sans fallback. |
| Ocean Optics SpectraSuite / OceanView / Jaz / CRAIC | partiel | Les 12 samples Ocean Optics committes sont golden-backed: textes SpectraSuite/OceanView/Jaz/JazIrrad/CRAIC/CSV/Master.Transmission, ProcSpec Linux/Windows/white-reference, JCAMP LINK via `jcamp-dx` et `.spc` OceanView route Galactic. Restent QE Pro/Maya/Apex, vrai `.spc` Ocean non-Galactic et rapports reference `lightr`/`pavo`. |
...
| `samples_local/` | 15 | 5 | 0 | Lectures OK: Indian Pines MATLAB v5, BUCHI cannabis, ARM MFRSR NetCDF, ARM SURFSPECALB derive, Allotrope ADF adfsee, 3 OMNIC `.srs` locaux et 7 exports Microtops MAN ASCII `.lev*`. Refus attendus: `_gt.mat` sidecar, NOAA `.lev2`, ARM AOSMET et PP Systems indices non raw/derives. |
...
| Allotrope ADF adfsee | `samples_local/allotrope_adf/adfsee_example.adf` | [`allotrope-open-source/adfsee`](https://gitlab.com/allotrope-open-source/adfsee) | ADF/ontology terms Allotrope, garder local | reader ADF experimental teste: 4 records depuis 3 data-cubes numeriques; RDF minimal mappe titres, axe temps, scale secondaire nm et absorbance mAU |
