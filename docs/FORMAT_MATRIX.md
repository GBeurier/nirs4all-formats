...
|---|---|---|---:|---:|---:|---:|---:|---|---|---|---|---|---|
| Foss NIRSystems / WinISI natif | Foss | `.NIR`, `.DA`, `.cal`, `.eqa` | 4 | 2 | 0 | 0 | 2 | diffusable cible | mineur | tres courant industrie | P2 sourcer | Natif `.cal`/`.nir` decode et valide vs export ISIscan `.txt` (fixtures reelles local-only + golden synthetique); `.DA`/`.eqa` non echantillonnes. | export ISIscan `.txt` |
...
| BUCHI NIRCal | BUCHI / Buhler | `.nir`, export JCAMP-DX | 4 | 1 | 1 | 1 | 1 | diffusable ciblé | moyen | courant industrie | P1 sourcer | `.nir` utile avec spectra, cibles, replicats et metadata spectre type prospectr; manquent fixture redistribuable cibles non nulles, `.cal` et variantes NIRMaster. | prospectr::read_nircal |
| JCAMP-DX | Vendor-neutral / IUPAC | `.jdx`, `.dx`, `.jcm`, `.jcamp` | 6 | 5 | 0 | 0 | 1 | diffusable | moyen | courant échange | P1 sourcer/completer | XYDATA/ASDF/NTUPLES/Ocean LINK/PEAK TABLE ok, avec verification des checkpoints X; LINK général réel reste à cadrer. | jcamp, SpectroChemPy, nmrglue, ChemoSpec, hyperSpec |
...
| VIAVI MicroNIR | VIAVI / JDSU | `.csv`, `.xlsx`, `.sam`, `.pri` | 4 | 3 | 0 | 0 | 1 | diffusable | mineur | courant handheld | P2 sourcer | Exports reels ok; natif `.sam` decode (125 canaux + 128 pixels bruts, fixtures local-only); `.pri` projet customer-only. | parseur texte, openpyxl |
...
| Ocean Optics SpectraSuite / OceanView / Jaz / CRAIC | Ocean Optics / Ocean Insight | `.txt`, `.csv`, `.jaz`, `.JazIrrad`, `.Master.Transmission`, `.ProcSpec`, `.jdx`, `.spc` | 11 | 8 | 0 | 3 | 0 | diffusable | moyen | tres courant | P2 compléter | Large couverture active; ajouter QE Pro/Maya/Apex si samples; Flame-NIR (InGaAs 950-1650 nm) in-family, export OceanView à sourcer. | lightr, pavo |
...
| Thermo Nicolet OMNIC | Thermo Nicolet | `.spa`, `.spg`, `.srs`, `.srsx` | 5 | 3 | 1 | 0 | 1 | diffusable | moyen | tres courant | P2 compléter | SPA/SPG/SRS utiles; `.srsx` absent, axe secondaire SRS à enrichir; Thermo Antaris II FT-NIR `.spa`/`.spg` in-family, fixture branded à sourcer. | SpectroChemPy, spa-on-python |
...
| ENVI / hyperspectral cubes | ENVI / Specim / HySpex / Headwall / NEON / AVIRIS | `.dat`, `.img` + `.hdr`, HDF5, `.lan`, `.mat` | 7 | 3 | 1 | 1 | 2 | diffusable ciblé | moyen | courant HSI | P2 sourcer | Cubes ENVI/AVIRIS ok avec ROI rectangulaire et masque sparse `(row, col)`; Specim/HySpex/NEON/HDF5 restent à sourcer. | spectral, rasterio, scipy |
...
| Perkin Elmer Spectrum / IR | PerkinElmer | `.sp`, `.fsm` | 2 | 1 | 0 | 0 | 1 | diffusable ciblé | moyen | courant | P3 sourcer | `.sp` ok (incl. FT-MIR Spectrum 10 a blocs residuels); `.fsm` imaging hors périmètre v1. | specio |
...
| Felix Instruments F-750 | Felix Instruments / CID Bio-Science | `.csv` (DataViewer) | 4 | 1 | 0 | 2 | 1 | diffusable ciblé | moyen | courant handheld terrain | P2 compléter | Export CSV absorbance (mango DMC, CC-BY) couvert via `csv_like`; modes reflectance/2e-dérivée et store natif à sourcer. | pandas, read.table |
...
|---|---|---|---|
| P2 | Foss NIRSystems / WinISI / ISIscan | Natif `.DA` et `.eqa` (les `.cal`/`.nir` sont decodes via fixtures local-only); idealement un jeu redistribuable `.cal`/`.nir` + export `.txt` du meme jeu. | Natif `.cal`/`.nir` decode et valide vs export ISIscan `.txt`; restent `.DA`/`.eqa` et une fixture publique redistribuable. |
...
| P1 | BUCHI NIRCal / NIRMaster | `.nir` redistribuable avec proprietes/cibles non nulles, fichiers `.cal` calibration-only, exports JCAMP-DX et variants NIRMaster/NIRFlex recents. | Le reader lit le `.nir` et ses metadata spectre principales, mais manque une fixture publique riche en cibles et variants. |
...
| P2 | VIAVI MicroNIR | Fichier projet natif `.pri` MicroNIR, plus exports CSV/XLSX du meme scan. | Les exports reels et le natif `.sam` passent; seul le projet `.pri` reste customer-only. |
...
| P2 | Thermo Antaris II FT-NIR | `.spa`/`.spg` natifs d'un Antaris II (RESULT/OMNIC) et export RESULT CSV/XLSX du meme scan. | Decode via le lecteur OMNIC/GRAMS/tabulaire; manque une fixture branded Antaris (datasets CC-BY: Mendeley 9z7dgdtggk tabac, h8mht3jsbz sol). |
| P2 | Ocean Optics Flame-NIR | Export OceanView (texte/`.ProcSpec`/`.jdx`) d'un Flame-NIR InGaAs 950-1650 nm. | Decode via `ocean_optics`; manque un export Flame-NIR couvrant l'axe InGaAs (vs les fixtures CCD actuelles). |
| P2 | Felix Instruments F-750 | Exports DataViewer Raw-Spectra (reflectance) et Interpolated-Spectra (2e-derivee), plus le store natif on-device. | Le CSV absorbance (mango DMC) est couvert; les autres modes d'export et le format natif manquent. |
...
|---|---|---|
| Foss NIRSystems / WinISI natif | partiel | Natif `.cal` (spectres + constituants) et `.nir` (spectres) decodes par le reader `foss_winisi`, sniffes par signature d'en-tete (mot de version + `ISIscan`/`NIRSystems`), jamais par extension — ce qui resout la collision `.nir` avec BUCHI NIRCal. Un record par echantillon: spectre absorbance (axe `nm` issu de la table de segments), valeurs de constituants en cibles, metadata (sample_number, product_code, timestamp, instrument). Valide vs les exports ISIscan `.txt` a la precision float32 (~5e-8) sur `yamtot_2026.cal` (18 ech.), `ando4_2026.cal` (20/9 const.), `D2026_20240328.cal` (18/5), `yamtot_2026.nir` (16/0). Golden synthetique `samples/foss_winisi/synthetic.cal` (CC0); fixtures reelles local-only (licence a definir). Restent `.DA`/`.eqa` non echantillonnes. |
...
| PP Systems UniSpec DC | partiel | Le `.SPU` synthetique est verrouille par golden et test semantique sur axe `nm`, metadata header, `channel_a_dn`/`channel_b_dn` raw et reflectance. Il manque une acquisition terrain reelle pour valider les deux canaux et metadata UniSpec DC. Les indices Arctic LTER locaux sont des produits derives, pas des spectres raw UniSpec; le reader les refuse maintenant avec le diagnostic `pp-systems-unispec-derived-indices`. |
| PP Systems UniSpec SC | partiel | Le `.SPT` synthetique est verrouille par golden et test semantique sur axe `nm`, metadata header, `dn_white`/`dn_target` raw et reflectance. Il manque une acquisition terrain reelle pour valider headers, units et metadata UniSpec SC. Les indices Arctic LTER locaux sont des produits derives, pas des spectres raw UniSpec; le reader les refuse maintenant avec le diagnostic `pp-systems-unispec-derived-indices`. |
...
| BUCHI NIRCal | partiel | Le chemin `.nir` lit spectra/wavenumbers/proprietes, GUID/version projet, index de replicats et metadata `Spectra Info` par spectre: GUID spectre, scans/resolution, geometrie wavenumber declaree, timestamps, createur/modificateur, device/serials, cellule/option et gain/temperatures quand presents. Les cibles non nulles sont validees localement sur `transpec_DEMO_cannabis.nir`, et les zeros restent numeriques des qu'une table de proprietes contient de vraies valeurs. Restent une fixture redistribuable avec cibles non nulles, `.cal` calibration-only, exports JCAMP-DX et variantes NIRMaster/NIRFlex. |
| JCAMP-DX | partiel | XYDATA/AFFN/ASDF/NTUPLES/LINK Ocean Optics et PEAK TABLE/PEAK ASSIGNMENTS top-level sont couverts, y compris fichiers multi-blocs (`nist_sucrose_ir.jdx` -> 2 records), NTUPLES FID a axe `time`, verification des checkpoints X de ligne et fixture sparse `peak_intensity`. Restent `LINK` generaux avec semantics heterogenes, peak tables reels pour conformance et plus de variantes NTUPLES. |
...
| Spectral Evolution / PSR | partiel | PSR DN brett + PSR-3500 grape leaf reels committes; reflectance `%`/fraction et DN sont types (`%`, `1`, `DN`), instrument/model/serial/mode/range/signaux source/GPS/date/time, canaux detecteurs, temperatures ref/target, integrations ref/target, batteries, moyennes, dark mode, foreoptic et `declared_column_count` sont promus. Le DN-only broken-but-valid reste signale par `sed_missing_reflectance_signal` / `missing_reflectance_signal`, et les incoherences `Channels`/`Columns` produisent des warnings dedies. Restent SR-3500 / SR-6500 firmware specifics, radiance/irradiance explicites et conformance `spectrolab`/`specdal`. |
...
| VIAVI MicroNIR | partiel | Reels CSV/XLSX MicroNIR 1700 committes et verrouilles par tests de lecture + probe (UvA forensic). Le natif `.sam` (conteneur `MNIR` .NET) est maintenant decode par le reader `viavi_micronir`: un record avec signal `absorbance` (125 canaux, axe `nm` interpole entre les bornes stockees ~908.1-1676.2) et `raw_single_beam` (128 pixels detecteur, axe Index), metadata riches (serial, operateur, integration, scans, timestamp). Sniff magic `\x04MNIR` (Definite) qui resout le faux positif Galactic-SPC. Golden synthetique `samples/viavi_micronir/synthetic_micronir.sam` (CC0); fixtures reelles local-only. Le projet natif `.pri` reste hors atteinte (customer-only). |
...
| Perkin Elmer Spectrum / IR | partiel | Le `.sp` PEPE mono-spectre reel `specio` est golden-backed et teste semantiquement. Le reader tolere maintenant les blocs top-level residuels que les exports FT-MIR Spectrum 10 ajoutent apres le bloc racine (bloc d'audit/CustomColumn), signales par le warning `perkin_elmer_reverse_engineered_trailing_blocks` sans affecter les donnees spectrales; 31 `.sp` FT-MIR reels valides vs leurs exports CSV (~5e-5). Robustesse durcie (compte de points negatif rejete, recursion de blocs plafonnee — erreurs propres, jamais de panic). Golden synthetique `samples/perkin_elmer/synthetic_trailing.sp` (CC0); corpus MIR reel local-only. Le statut reste partiel parce que la ligne inclut `.fsm` Spotlight imaging, detecte/refuse comme hors v1, et parce qu'il manque des variantes PE NIR/Lambda. |
...
| Felix Instruments F-750 | partiel | Le CSV DataViewer absorbance (mango DMC, CC-BY 4.0, Mendeley 10.17632/46htwnp833.1) est golden-backed via `csv_like` (`csv_felix_f750_mango_slice`, 26 records, axe `nm` 285-1200, 306 points, cible `DM`, metadata cultivar/region/saison). Comme VIAVI MicroNIR et NeoSpectra, aucun lecteur dedie n'est requis. Restent les modes DataViewer Raw-Spectra (reflectance) et Interpolated-Spectra (2e-derivee), et le store natif on-device. |
...
| Microtops Sun Photometer | partiel | MAN NetCDF reel committe et teste (PANGAEA MSM114/2, CC-BY-4.0). La decouverte generique `aot_<nm>` reste; quand `hdf5-reader` 0.5 echoue a resoudre les attributs partages de ce layout, le reader bascule sur un decodeur generique base sur `DataLayout::Contiguous` (scan des link records de la fractal heap + `get_or_parse_header(addr)` + lecture brute), sans table de hash ni offsets fixture. Il emet `microtops_man_netcdf_contiguous_layout_fallback` et, pour les global attributes string fixes, `microtops_man_netcdf_global_attributes_byte_scan`. Les 7 exports AERONET MAN ASCII locaux `.lev10/.lev15/.lev20` sont testes avec AOD et AOD-STD; les signaux primaires AOT sont types `aerosol_optical_thickness`, et `aot_std` est type `uncertainty`. Restent un vrai `.TXT` legacy redistribuable et la disparition complete du fallback une fois que `hdf5-reader` resoudra correctement les attributs partages NetCDF4. |
...
| BUCHI NIRCal cannabis | `samples_local/buchi_nircal/transpec_DEMO_cannabis.nir` | orellano-c/transpec_info | licence non clarifiee pour redistribution du fixture -> en local seulement | reader local teste: 105 spectres, axe 1501 wavenumbers, 35 groupes de 3 replicats, cibles non nulles `CBDA`/`THCA`, comments/timestamps/device/serials et chemin sans gain/temperature |
...
| PP Systems Arctic LTER indices | `samples_local/pp_systems/*.csv/.xlsx` | Arctic LTER / EDI | dataset local non committe | produit derive NDVI/EVI/PRI/WBI/Chl/LAI + metadata; refus explicite `pp-systems-unispec-derived-indices`; ne ferme pas le manque de raw `.SPT/.SPU` ou de table reflectance wavelength-indexed |
...
| Foss `.NIR/.DA/.cal/.eqa` natif | Natif `.cal`/`.nir` decode (reader `foss_winisi`) et valide vs export ISIscan `.txt`, mais sur fixtures reelles local-only (licence a definir); aucune fixture binaire publique redistribuable trouvee. `.DA`/`.eqa` non echantillonnes. |
...
| PP Systems UniSpec `.SPT/.SPU` reel raw | Aucune fixture raw `.spu/.spt` publique; `samples_local/pp_systems/` contient seulement des indices derives Arctic LTER — non commit. Le workbook reference un fichier reflectance data scan separe qui n'est pas present localement. |
...
| VIAVI MicroNIR `.pri` natif | Format projet binaire, customer-only; le natif `.sam` (sample) est decode (reader `viavi_micronir`, fixtures local-only) et les exports CSV/XLSX reels sont couverts via UvA forensic. Seul le projet `.pri` reste hors atteinte. |
