...
| Foss NIRSystems / WinISI `.DA`, `.eqa` | Native `.cal`/`.nir` are now decoded (see Partial Real Coverage); `.DA`/`.eqa` remain undecoded. | No open `.DA`/`.eqa` fixture; these auxiliary database/equation files are unsampled. | Contribute a `.DA`/`.eqa` companion alongside its `.cal`/`.nir` plus an ISIscan text export. |
...
| VIAVI MicroNIR `.pri` project container | Not decoded; native `.sam` measurements (see Partial Real Coverage) and MicroNIR CSV matrices are supported. | Customer-only project format. | Export CSV or save individual `.sam` measurements. |
| WiTec `.wip` / `.wid` variants outside `WIT_PR06` TDGraph | The committed `Sa4.wip` layout decodes experimentally with Raman-shift axis and map coordinates; signed `WIT^` and unknown `WIT_PR06` layouts are refused. WiTec ASCII export is supported. | Only one redistributable binary map layout is available, and no paired ASCII export exists for the same project. | Export from WiTec Project/FIVE as ASCII text; broaden native decode only with matching `.wip` plus reference export. |
...
| Perkin Elmer | `.sp` single spectra, including FT-MIR Spectrum 10 exports that append a trailing audit/history block after the `DataSet` root. | `.fsm` Spotlight imaging is intentionally out of v1; PE Lambda NIR-specific variants need fixtures. The Spectrum 10 audit block is recorded structurally but its content is not surfaced. |
| Foss NIRSystems / WinISI | Native `.cal` (spectra + constituent reference targets) and spectra-only `.nir`, sniffed by binary header signature and validated against ISIscan `.txt` exports to float32 precision; constituents become record targets. A committed synthetic `.cal` golden plus local-only real vendor files. | `.DA` database and `.eqa` equation companions remain unsampled. |
| VIAVI MicroNIR | Native `.sam` measurements with calibrated absorbance (125 channels) and raw single-beam (128 pixels) signals, sniffed by the `MNIR` magic; a committed synthetic `.sam` golden plus local-only real vendor files. | The `.pri` project container is still customer-only/blocked. The absorbance wavelength axis is interpolated from the stored endpoints (vendor stores no per-pixel wavelength vector). |
| BUCHI NIRCal | One committed `.nir` transfer file with spectra/wavenumbers/property schema, project GUID/version, replicate metadata and per-spectrum `Spectra Info` metadata, plus a local-only cannabis `.nir` validating non-zero `CBDA`/`THCA` targets and 3 replicate spectra per sample. | Redistributable non-zero target fixture, `.cal` calibration-only files, JCAMP-DX exports and NIRMaster/NIRFlex variants. |
