...
| BUCHI NIRCal | One committed `.nir` transfer file with spectra/wavenumbers/property schema, project GUID/version and singleton replicate metadata, plus a local-only cannabis `.nir` validating non-zero `CBDA`/`THCA` targets and 3 replicate spectra per sample. | Redistributable non-zero target fixture, `.cal` calibration-only files, NIRMaster variants. |
