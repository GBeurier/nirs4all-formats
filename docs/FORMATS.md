...
| PP Systems UniSpec SC | `.SPT` | ascii | 🟠 | Ad-hoc | Axis-first text export now covered by `spectral_table`; limited literature; SPECCHIO claims support. Local Arctic LTER indices are derived products and do not close the raw `.SPT` sample gap. |
| PP Systems UniSpec DC | `.SPU` | ascii | 🟠 | Ad-hoc | Axis-first text export now covered by `spectral_table`. Local Arctic LTER indices are derived products and do not close the raw `.SPU` sample gap. |
...
| Allotrope ADF | `.adf` | hdf5 + triplestore | 🟡 | Allotrope Foundation SDK, `adfsee` | Experimental local-only reader for numeric `/data-cubes`; RDF semantics, ontology units and vendor ADFs remain unresolved. |
