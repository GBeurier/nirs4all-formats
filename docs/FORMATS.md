...
| SiWare API JSON / CSV companion | `.json` with `measurement.wavelengths` / `measurement.absorbance`; `.csv` axis-first export | Experimental | One-measurement NeoSpectra-style JSON payloads; predictions become targets. The synthetic CSV stream is covered through `row-spectral-table`. |
...
| ENVI Spectral Library / Standard cubes | `.sli` + `.hdr`, `.img`/`.dat` + `.hdr` | Experimental | Paired sidecar reader for `file type = ENVI Spectral Library`, one-band BSQ float32/float64 payloads, plus ENVI Standard BSQ/BIL/BIP cube expansion to one point spectrum per pixel with optional rectangular ROI windows or sparse `(row, col)` pixel masks. |
| ERDAS LAN / AVIRIS 92AV3C | `.lan` + `.spc` + optional `.GIS` | Experimental partial | Classic Indian Pines AVIRIS cube, 145 x 145 x 220 u16 BIL payload, expanded to one raw-count spectrum per pixel with optional rectangular ROI windows or sparse `(row, col)` pixel masks, plus ground-truth class labels when present. |
...
| JCAMP-DX | `.jdx`, `.dx`, `.jcm` | Experimental partial | Single-block `XYDATA=(X++(Y..Y))` with plain AFFN plus PAC/SQZ/DIF/DUP ASDF ordinate decoding and line-start X checkpoint warnings, NMR `NTUPLES` real/imaginary pages with frequency/time axes, Ocean Optics `LINK`/`XYPOINTS` sample-dark-reference blocks, and top-level sparse `PEAK TABLE` / `PEAK ASSIGNMENTS` records. Broader heterogeneous `LINK` variants are pending. |
...
| Spectral Evolution SED | `.sed` | Experimental | Header key/value metadata plus wavelength and signal columns; DN-only files are flagged when no reflectance column exists. DN, percent/fraction reflectance, instrument/GPS/date/time and source-signal metadata are promoted. |
...
| Perkin Elmer Spectrum / IR | `.sp` | Experimental | `PEPE` block reader for single-spectrum `.sp` files, tolerating the trailing audit/history block that FT-MIR Spectrum 10 exports append after the `DataSet` root; `.fsm` imaging is detected but out of scope for v1. |
| BUCHI NIRCal | `.nir` | Experimental | `NIRCAL Project File` section reader for the committed 20-sample foliar-transfer fixture and a local 105-spectrum cannabis transfer fixture; property names are mapped to targets, repeated sample ids get replicate metadata, committed zero-valued properties are emitted as nulls and local non-null `CBDA`/`THCA` targets are preserved. |
...
| Horiba LabSpec / JobinYvon | `.xml`, `.txt` LabSpec exports | Experimental | LSX XML single spectra, range exports, linescans and maps plus LabSpec two-column, series-row and map-row text exports. eV axes are typed as `Energy`. |
| WiTec WIP / WID | `.wip`, `.wid`, `.txt` | Experimental partial | WiTec ASCII exports load through `row-spectral-table`; `WIT_PR06` TDGraph maps matching `Sa4.wip` decode to raw-count spectra with a Raman-shift axis and derived map coordinates. Legacy `WIT^` and unknown project layouts are refused explicitly. |
| Renishaw WDF | `.wdf` | Experimental | `WDF1` chunk reader for spectral payloads via `DATA`, `XLST` and `YLST`; `ORGN`/`WMAP` navigation metadata adds spatial X/Y/Z, FocusTrack Z, elapsed time, map dimensions and map indices. `WHTL` JPEG image metadata and observed `MAP ` PSET `dataRange` analysis values are exposed conservatively. |
| Princeton TriVista TVF | `.tvf` | Experimental | XML `Frame` payload reader for committed single spectra, multi-frame spectra, line scans, maps, time series and Step-and-Glue acquisitions. `xDim/Calibration` drives the spectral axis with length checks; `InfoSerialized` X/Y axes, detector fields and numbered spectrometers become metadata, without inventing missing spatial units. |
| DigitalSurf MountainsMap | `.sur`, `.pro` | Experimental | Fixed-header object reader for committed spectra, multi-spectrum profiles, hyperspectral maps and surface profiles, including `DSCOMPRESSED` zlib stream payloads. Maps/surfaces expose normalized spatial indices, dimensions and axis-order metadata. |
...
| Perkin Elmer Spectrum / IR | `.sp`, `.fsm` | bin | 🟡 | Py: `specio` | Current native reader covers `PEPE` single-spectrum `.sp` block files with f64 ordinates, including FT-MIR Spectrum 10 exports that append a trailing audit/history block after the `DataSet` root (the audit block is recorded structurally but its content is not surfaced). `.fsm` is an imaging variant we detect and refuse as out-of-scope for v1. |
| Foss NIRSystems / WinISI | `.NIR`, `.DA`, `.cal`, `.eqa` | bin | 🟠 | None reliable (reverse-engineered) | **⚠ `.NIR` extension is shared with BUCHI NIRCal — never route by extension alone, always sniff the header signature.** Native `.cal` (spectra + constituent reference targets) and spectra-only `.nir` are now decoded from the WinISI header signature and validated against ISIscan `.txt` exports to float32 precision; the committed WinISI text matrix export remains covered by `spectral_matrix`. `.DA`/`.eqa` companions remain undecoded. |
...
| VIAVI MicroNIR (handheld) | native `.sam`, CSV/XLSX export (`.csv`, `.xlsx`) | bin / ascii/tabular | ✅ | Any CSV/Excel loader | Committed spectral matrix CSV export and real MicroNIR 1700 XLSX exports are covered. Native `.sam` measurements are now decoded from the `MNIR` magic into calibrated absorbance (125 channels) plus raw single-beam (128 pixels); the absorbance wavelength axis is interpolated from the stored endpoints. Native `.pri` project files remain out of scope without a public fixture/spec. |
...
| Allotrope ADF | `.adf` | hdf5 + triplestore | 🟡 | Allotrope Foundation SDK, `adfsee` | Experimental local-only reader for numeric `/data-cubes`; minimal RDF component mapping covers cube titles, typed seconds axes, secondary nm scales and absorbance units, while full ontology mapping and vendor ADFs remain unresolved. |
...
Standard and AVIRIS LAN now support both rectangular row/column windows and
sparse `(row, col)` pixel masks through the Rust options API and CLI, so
callers can extract arbitrary subsets without materializing the full cube.
Real-scale NEON, Specim, HySpex, Headwall and AVIRIS-NG fixtures still need to
be sourced before promoting these readers further.
