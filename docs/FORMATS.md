...
| Thermo Nicolet OMNIC | `.SPA`, `.spg` | Experimental | Reverse-engineered key-table reader for single-spectrum `.SPA` and grouped `.SPG` files. `.srs` time-series files are sniffed but not decoded yet. |
...
| Thermo Nicolet OMNIC | `.spa`, `.spg`, `.srs`, `.srsx` | bin | 🟡 | Py: [`spectrochempy.read_omnic()`](https://www.spectrochempy.fr/reference/generated/spectrochempy.read_omnic.html), `lerkoah/spa-on-python` (.spa only) | Current native reader covers `.spa` single spectra and `.spg` grouped spectra via the reverse-engineered key table. `.srs/.srsx` time series remain pending because variants differ across OMNIC releases. |
